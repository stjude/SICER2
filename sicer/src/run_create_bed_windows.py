...
def create_bed_windows(chrom, chrom_length):
    file_save_name = chrom + '.windows'
    syntax = 'echo "%s\\t%s" > %s.temp ; bedtools makewindows -g %s.temp -w 50 > %s ; rm -rf %s.temp' % (chrom, chrom_length, chrom, chrom, file_save_name, chrom)
